...
int Ass_01_Main(void)
{
  ...
  char *parse_string[] = {"one two three"," one  two three  ","    string     test analysis apple",NULL};
	    ...
	    if (coeff_num > 0)
	    	{
	    		...
	    		for(int k=0; k<=coeff_num; k++) printf(", coeff_values[%d] = %f", k, coeff_values[k]);
	    	}
	    printf("\n");
	  }
	  free(coeff_values);
  ...
  // Question 2
  {
	...
	while (filename_audio[i] != NULL)
    {
	  ...
	  if (read_pcm_wavefile(&header, &data, filename_audio[i])==0)
	  {
        ...
        {
			snprintf(filename_output,100,"%s-rewrite.wav",filename_audio[i]);
